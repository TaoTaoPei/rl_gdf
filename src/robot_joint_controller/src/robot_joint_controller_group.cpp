...
{
    RCLCPP_INFO(get_node()->get_logger(), "Called on_configure()");
    name_space_ = get_node()->get_namespace();
    RCLCPP_WARN(get_node()->get_logger(), "name_space_: %s", name_space_.c_str());
    ...
    {
        std::string robot_description = future.get()->values[0].string_value; // 解析urdf
        // RCLCPP_WARN(get_node()->get_logger(), "robot_description: %s", robot_description.c_str());
        urdf::Model urdf;
...
{
    printf("*************Called command_interface_configuration()*****************\n");
...
{
    printf("*************Called state_interface_configuration()*****************\n");
...
{
    RCLCPP_INFO(get_node()->get_logger(), "Called on_activate()");
    ...
    for (int index = 0; index < joint_names_.size(); ++index)
    {
        ...
        RCLCPP_INFO(get_node()->get_logger(), "init_pos %f", init_pos);
...
{
    RCLCPP_INFO(get_node()->get_logger(), "Called on_deactivate()");
    last_command_ = robot_msgs::msg::RobotCommand();
    last_command_.motor_command.resize(joint_names_.size());
    last_state_ = robot_msgs::msg::RobotState();
    last_state_.motor_state.resize(joint_names_.size());
