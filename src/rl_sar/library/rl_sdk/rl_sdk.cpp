...
torch::Tensor RL::ComputeObservation()
{
    ...
    /***
     observations: ["ang_vel_body", "gravity_vec", "commands", "dof_pos", "dof_vel", "actions"]
    ***/
    ...
    // std::cout << "obs size: " << obs.sizes() << std::endl; // [1, 45]
...
void RL::StateController(const RobotState<double> *state, RobotCommand<double> *command)
{
    ...
    */
    ...
    else if (this->running_state == STATE_RL_RUNNING)
    {
        ...
        for (int i = 0; i < this->params.num_of_dofs; ++i)
        {
            command->motor_command.q[i] = this->output_dof_pos[0][i].item<double>(); // 强化学习输出位置
            command->motor_command.dq[i] = 0;
...
void RL::ReadYaml(std::string robot_name)
{
    ...
    if (this->params.framework == "isaacgym"){
        ...
        this->params.default_dof_pos = torch::tensor(ReadVectorFromYaml<double>(config["default_dof_pos"], this->params.framework, cols, rows)).view({1, -1});
        this->params.joint_controller_names = ReadVectorFromYaml<std::string>(config["joint_controller_names"], this->params.framework, cols, rows);
    }
    if(true){
        std::cout << "================ RL Parameters ================" << std::endl;
        std::cout << "Model Name: " << this->params.model_name << std::endl;
        std::cout << "Framework: " << this->params.framework << std::endl;
        std::cout << "dt: " << this->params.dt << std::endl;
        std::cout << "decimation: " << this->params.decimation << std::endl;
        std::cout << "num_observations: " << this->params.num_observations << std::endl;

        std::cout << "observations: ";
        for (const auto &obs : this->params.observations)
            std::cout << obs << " ";
        std::cout << std::endl;

        std::cout << "observations_history: ";
        for (const auto &h : this->params.observations_history)
            std::cout << h << " ";
        std::cout << std::endl;

        std::cout << "clip_obs: " << this->params.clip_obs << std::endl;

        std::cout << "clip_actions_lower: " << this->params.clip_actions_lower << std::endl;
        std::cout << "clip_actions_upper: " << this->params.clip_actions_upper << std::endl;

        std::cout << "action_scale: " << this->params.action_scale << std::endl;
        std::cout << "hip_scale_reduction: " << this->params.hip_scale_reduction << std::endl;

        std::cout << "hip_scale_reduction_indices: ";
        for (const auto &idx : this->params.hip_scale_reduction_indices)
            std::cout << idx << " ";
        std::cout << std::endl;

        std::cout << "num_of_dofs: " << this->params.num_of_dofs << std::endl;

        std::cout << "lin_vel_scale: " << this->params.lin_vel_scale << std::endl;
        std::cout << "ang_vel_scale: " << this->params.ang_vel_scale << std::endl;
        std::cout << "dof_pos_scale: " << this->params.dof_pos_scale << std::endl;
        std::cout << "dof_vel_scale: " << this->params.dof_vel_scale << std::endl;

        std::cout << "commands_scale: " << this->params.commands_scale << std::endl;

        std::cout << "rl_kp: " << this->params.rl_kp << std::endl;
        std::cout << "rl_kd: " << this->params.rl_kd << std::endl;
        std::cout << "fixed_kp: " << this->params.fixed_kp << std::endl;
        std::cout << "fixed_kd: " << this->params.fixed_kd << std::endl;
        std::cout << "torque_limits: " << this->params.torque_limits << std::endl;
        std::cout << "default_dof_pos: " << this->params.default_dof_pos << std::endl;

        std::cout << "joint_controller_names: ";
        for (const auto &name : this->params.joint_controller_names)
            std::cout << name << " ";
        std::cout << std::endl;
